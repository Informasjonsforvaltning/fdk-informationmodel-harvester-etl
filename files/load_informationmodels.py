...
with open(args.outputdirectory + 'informationmodels_transformed.json') as transformed_file:
    ...
    for mongo_id in transformed_json:
        ...
        print(db.informationmodel.find_one_and_update({'_id': mongo_id},  {'$set': to_be_updated}))
        # print(db.informationmodel.find_one_and_update({'_id': mongo_id},  {'$set': {"fdkId": str(values["fdkId"])}}))
        # print(db.informationmodel.find_one_and_update({'_id': mongo_id},  {'$set': {"issued": bson.Int64(int(values["issued"]))}}))
        # print(db.informationmodel.find_one_and_update({'_id': mongo_id},  {'$set': {"modified": bson.Int64(int(values["modified"]))}}))
